...
function createData(node) {
  counter += 1;
  return { id: counter, name: node.name, group: node.group, uid: node.uid };
}
...
class EnhancedTable extends React.Component {
  ...
  static getDerivedStateFromProps({ nodes }, prevState) {
    const l = [];
    counter = 0;
    nodes.map(node => l.push(createData(node, 5, 5)));
    return {
      data: l,
      selected:
        prevState.data.length !== nodes.length ? [] : prevState.selected,
    };
  }
  ...
  handleDelete = () => {
    ...
    const justIntents = selected.map(a => a.node);
