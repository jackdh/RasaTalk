...
function createData(intent, coverage, hits) {
  counter += 1;
  return { id: counter, intent: intent.name, coverage, hits };
}
...
class EnhancedTable extends React.Component {
  ...
  static getDerivedStateFromProps({ intents }, prevState) {
    const l = [];
    counter = 0;

    intents.map(intent => l.push(createData(intent, 5, 5)));
    return {
      data: l,
      selected:
        prevState.data.length !== intents.length ? [] : prevState.selected,
    };
  }
  ...
  handleDelete = () => {
    ...
    const justIntents = selected.map(a => a.intent);
